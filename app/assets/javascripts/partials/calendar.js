...
function drawCalendar (cal) {
  ...
  function pushLine () {
    ...
    for (var i = nextRow.events.length - 1; i >= 0; i--) {
      evt = "<div class='eventrow'><div class='mainrow'><div class='eventtime'>" + formatTime(nextRow.events[i].Hour,nextRow.events[i].Minute) + "</div>" +
             "<div class='eventtitle'>" + nextRow.events[i].title + "</div></div><div class='eventsummmary'>" + nextRow.events[i].summary + "</div></div>" + evt;
    };
